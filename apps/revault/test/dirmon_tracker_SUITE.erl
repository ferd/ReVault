...
conflict_creation(Config) ->
    ...
    WorkFile = "work",
    ConflictMarker = "work.conflict",
    ConflictA = "work." ++ hexname("a"),
    ConflictB = "work." ++ hexname("b"),
    AbsWorkFile = filename:join([Dir, WorkFile]),
    AbsConflictMarker = filename:join([Dir, ConflictMarker]),
    AbsConflictA = filename:join([Dir, ConflictA]),
    AbsConflictB = filename:join([Dir, ConflictB]),
    ...
    ok = revault_dirmon_tracker:conflict(Name, WorkFile, AbsWorkFile, {Vsn1, HashA}),
    ?assertEqual({ok, <<"a">>}, file:read_file(AbsWorkFile)),
    ?assertEqual({ok, <<"a">>}, file:read_file(AbsConflictA)),
    ?assertEqual({ok, hex(<<"a">>)}, file:read_file(AbsConflictMarker)),
    %% Add more conflict files
    ok = file:write_file(TmpFile, <<"b">>),
    ok = revault_dirmon_tracker:conflict(Name, WorkFile, TmpFile, {Vsn1, hash(<<"b">>)}),
    ?assertEqual({ok, <<"a">>}, file:read_file(AbsWorkFile)),
    ?assertEqual({ok, <<"a">>}, file:read_file(AbsConflictA)),
    ?assertEqual({ok, <<"b">>}, file:read_file(AbsConflictB)),
    ?assertEqual({ok, iolist_to_binary([
            %% per-hash alphabetical order
            hex(<<"b">>), "\n", hex(<<"a">>)
        ])},
        file:read_file(AbsConflictMarker)
    ),
    ...
    ok = file:write_file(AbsWorkFile, <<"b">>),
    ok = revault_dirmon_event:force_scan(Name, 5000),
...
conflict_resolution_orderly(Config) ->
    ...
    ok = file:delete(AbsConflictA),
    ok = file:delete(AbsConflictB),
    ok = revault_dirmon_event:force_scan(Name, 5000),
    ?assertEqual({ok, <<"a">>}, file:read_file(AbsWorkFile)),
    ?assertEqual({ok, iolist_to_binary([
            hex(<<"b">>), "\n", hex(<<"a">>)
    ...
    ok = file:write_file(AbsWorkFile, <<"b">>),
    ok = revault_dirmon_event:force_scan(Name, 5000),
    ok = file:delete(AbsConflictMarker),
    ok = revault_dirmon_event:force_scan(Name, 5000),
...
conflict_resolution_marker_first(Config) ->
    ...
    ?assertNotEqual(Vsn1, Vsn2),
    ?assertEqual({ok, <<"a">>}, file:read_file(AbsWorkFile)),
    ?assertEqual({error, enoent}, file:read_file(AbsConflictA)),
    ?assertEqual({error, enoent}, file:read_file(AbsConflictB)),
    ok.
...
conflict_resolution_drop_conflict(Config) ->
    ...
    ?assertNotEqual(Vsn1, Vsn2),
    ?assertEqual({error, enoent}, file:read_file(AbsWorkFile)),
    ?assertEqual({error, enoent}, file:read_file(AbsConflictA)),
    ?assertEqual({error, enoent}, file:read_file(AbsConflictB)),
    ok.
...
update_file(Config) ->
    ...
    ?assertEqual({ok, <<"b">>}, file:read_file(AbsWorkFile)),
...
update_new(Config) ->
    ...
    TmpFile = filename:join([?config(tmp_dir, Config), "work"]),
    WorkFile = "work",
    AbsWorkFile = filename:join([Dir, "work"]),
    ok = revault_dirmon_event:force_scan(Name, 5000),
    %% Create the remote file to merge
    ok = file:write_file(TmpFile, <<"b">>),
    %% cheating with internals
...
update_conflict(Config) ->
    ...
    ConflictA = "work." ++ hexname("a"),
    ConflictB = "work." ++ hexname("b"),
    ConflictC = "work." ++ hexname("c"),
    AbsConflictMarker = filename:join([Dir, ConflictMarker]),
    AbsConflictA = filename:join([Dir, ConflictA]),
    AbsConflictB = filename:join([Dir, ConflictB]),
    AbsConflictC = filename:join([Dir, ConflictC]),
    ok = revault_dirmon_event:force_scan(Name, 5000),
    ...
    ?assertMatch({error, enoent}, file:read_file(AbsConflictC)),
    ...
    ?assertMatch({error, enoent}, file:read_file(AbsConflictA)), % created on opposed end sync
    ?assertMatch({ok, _}, file:read_file(AbsConflictB)),
    ?assertMatch({ok, _}, file:read_file(AbsConflictC)),
    ok.
...
update_conflict_resolve(Config) ->
    ...
    ?assertEqual({error, enoent}, file:read_file(AbsConflictMarker)),
    ?assertEqual({error, enoent}, file:read_file(AbsConflictB)),
...
update_conflict_older(Config) ->
    ...
    ?assertMatch({error, enoent}, file:read_file(AbsConflictA)), % created on opposed end sync
    ?assertMatch({ok, _}, file:read_file(AbsConflictB)),
    ?assertEqual({error, enoent}, file:read_file(AbsConflictC)),
    ok.
